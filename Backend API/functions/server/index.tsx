...
async function resolveAgentMatricule(userId: string): Promise<string> {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    // SECURITY: Use CSPRNG for agent matricule generation to prevent predictability
    const randBuf = new Uint32Array(1);
    crypto.getRandomValues(randBuf);
    const n = 1000 + (randBuf[0] % 9000);
...
app.post(`${PREFIX}/phone/otp/send`, async (c) => {
  try {
    ...
    // SECURITY: Use CSPRNG for 6-digit OTP generation to prevent PRNG prediction attacks
