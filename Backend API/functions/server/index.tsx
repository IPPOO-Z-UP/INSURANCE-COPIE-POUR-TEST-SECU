...
function secureRandomSuffix(len: number = 6): string {
  const chars = "abcdefghijklmnopqrstuvwxyz0123456789";
  let res = "";
  for (let i = 0; i < len; i++) {
    res += chars[secureRandomInt(0, chars.length - 1)];
  }
  return res;
}
...
async function adminAudit(c: any, admin: { username: string; role?: string }, action: string, meta: Record<string, any> = {}) {
  try {
    ...
    const id = `aa_${Date.now()}_${secureRandomSuffix(6)}`;
...
async function audit(uid: string, action: string, meta: Record<string, any> = {}) {
  try {
    ...
    list.unshift({
      id: `a_${Date.now()}_${secureRandomSuffix(6)}`,
      action,
      meta,
...
function notify(notifications: any[], title: string, body: string, type = "info", to?: string) {
  notifications.unshift({
    id: `n_${Date.now()}_${secureRandomSuffix(6)}`,
    title,
    body,
    type,
    read: false,
...
app.post(`${PREFIX}/payments/initiate`, async (c) => {
  ...
  try {
    ...
    if (paymentId) {
      ...
      payment = {
        id: `p_${Date.now()}_${secureRandomSuffix(6)}`,
        contractId: contractId ?? null,
        amount,
        currency: "XOF",
        method: "mobile_money",
...
app.post(`${PREFIX}/admin/dev/seed-demo`, async (c) => {
  ...
  try {
    ...
    const password = `Demo!${secureRandomSuffix(8)}`;
...
app.post(`${PREFIX}/agent/visits/:uid`, async (c) => {
  ...
  try {
    ...
    const list = ((await kv.get(key)) ?? []) as any[];
    const entry = {
      id: `v_${Date.now()}_${secureRandomSuffix(6)}`,
      agentId: r.agent.id, matricule: r.agent.matricule,
...
async function logWebhookEvent(opts: {
  ...
  try {
    const id = `wh_${Date.now()}_${secureRandomSuffix(6)}`;
