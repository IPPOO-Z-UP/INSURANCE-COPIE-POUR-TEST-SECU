...
/**
 * Cryptographically secure alphanumeric suffix of given length.
 */
function secureRandomSuffix(len: number): string {
  ...
  crypto.getRandomValues(array);
  let res = "";
  for (let i = 0; i < len; i++) res += chars[array[i] % chars.length];
  return res;
}

/**
 * Logs the real error internally and returns a sanitized message to the client
 * to prevent information leakage (e.g. stack traces, internal paths).
 */
function safeError(c: any, err: unknown, publicMsg: string, status: number = 500) {
  console.log(`[SENTINEL] Internal error during "${publicMsg}":`, err);
...
app.post(`${PREFIX}/profile/avatar`, async (c) => {
  ...
  try {
    ...
    return safeError(c, err, "Erreur lors du téléchargement de l'avatar");
  }
});
...
app.put(`${PREFIX}/me`, async (c) => {
  ...
  try {
    ...
    return safeError(c, err, "Erreur de mise à jour du profil");
  }
});
...
app.post(`${PREFIX}/claims`, async (c) => {
  ...
  try {
    ...
    return safeError(c, err, "Erreur de création du sinistre");
  }
});
