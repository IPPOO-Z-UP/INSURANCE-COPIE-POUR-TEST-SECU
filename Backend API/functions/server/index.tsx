...
/**
 * Generates a cryptographically secure random alphanumeric suffix of specified length.
 * Replaces insecure Math.random().toString(36) patterns.
 */
...
/**
 * Masks internal error details from the client while logging them on the server.
 * Prevents information leakage via stack traces or database errors.
 */
function safeError(c: any, err: any, message: string, status: number = 500) {
  console.error(`[Sentinel] Internal error:`, err);
...
app.post(`${PREFIX}/profile/avatar`, async (c) => {
  ...
  try {
    ...
    return safeError(c, err, "Erreur lors de l'upload de l'avatar");
  }
});
