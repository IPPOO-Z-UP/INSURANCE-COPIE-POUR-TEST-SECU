...
/** Secure integer in [min, max] range using Web Crypto */
...
/** Secure alphanumeric suffix using Web Crypto */
function secureRandomSuffix(length = 4): string {
  const chars = "abcdefghijklmnopqrstuvwxyz0123456789";
  const array = new Uint8Array(length);
  crypto.getRandomValues(array);
  let res = "";
  for (let i = 0; i < length; i++) res += chars[array[i] % chars.length];
  return res;
}

/** Standard internal error masking to prevent info leakage */
function safeError(c: any, err: unknown, status = 500) {
