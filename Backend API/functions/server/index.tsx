...
app.post(`${PREFIX}/phone/otp/send`, async (c) => {
  try {
    ...
    // Use cryptographically secure randomness for OTP generation to prevent PRNG predictability attacks
    const otpBuf = new Uint32Array(1);
    let num: number;
    do {
      crypto.getRandomValues(otpBuf);
      num = otpBuf[0];
    } while (num >= 4294800000); // Rejection sampling (4772 * 900000) to avoid modulo bias
